...
const handleWebSocket = (ws: WebSocket, dbFilename: string, duckdbPath: string) => {
  const duckdbProcess = new Deno.Command(duckdbPath, { args: [dbFilename], stdin: "piped", stdout: "piped", stderr: "piped" }).spawn();
  const stdinWriter = duckdbProcess.stdin.getWriter();

  ws.onmessage = async ({ data }) => { if (typeof data === "string") await stdinWriter.write(new TextEncoder().encode(data + "\n")); };

  ["stdout", "stderr"].forEach(stream => 
    (async () => {
      for await (const chunk of duckdbProcess[stream as "stdout" | "stderr"]) {
        ws.send(JSON.stringify({ type: stream, data: new TextDecoder().decode(chunk) }));
      }
    })()
  );

  ws.onclose = () => { stdinWriter.close(); duckdbProcess.kill(); };
};
...
const duckdbPath = await ensureDuckDB().catch(error => { console.error("Failed to ensure DuckDB:", error); Deno.exit(1); });
